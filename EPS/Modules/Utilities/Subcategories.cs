...
namespace Modules.Utilities
{
    public class Subcategories
    {
        ...
        public DataTable GetSubcategory(string sPermitCode, string sArn)
        {
            ...
            sQuery += $" and S.struc_code like (select distinct '%' || struc_code || '%' from application_que where arn = '{sArn}') ";
            sQuery += $" and S.scope_code like (select distinct '%' || scope_code || '%' from application_que where arn = '{sArn}') order by S.fees_code";
            var epsrec = db.Database.SqlQuery<SUBCATEGORIES>(sQuery);

            foreach (var items in epsrec)
            {
                dataTable.Rows.Add(false,items.FEES_DESC,items.FEES_CODE,items.FEES_MEANS,items.FEES_UNIT,items.AREA_NEEDED, items.CUMULATIVE,0,0.00,items.CATEGORY_CODE,items.SCOPE_CODE);
            }
            return dataTable;
        }
    }
