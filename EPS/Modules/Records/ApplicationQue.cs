...
namespace Modules.Records
{
    public class ApplicationQue
    {
        ...
        private Nullable<DateTime> PROP_START { get; set; }
        private Nullable<DateTime> PROP_COMPLETE { get; set; }
        private Nullable<DateTime> DATE_APPLIED { get; set; }
        private Nullable<DateTime> DATE_ISSUED { get; set; }
        private string MEMO { get; set; }
        private int MAIN_APPLICATION { get; set; }

        public ApplicationQue()
        {
            this.Clear();
        }
        ...
        public void GetRecord(string sARN)
        {
           ...
           var db = new EPSConnection(dbConn);
            ...
            sQuery = $"SELECT * FROM application_que WHERE arn = '{sARN}'";
            var trsrec = db.Database.SqlQuery<APPLICATION_QUE>(sQuery);
        ...
        public ApplicationQue(string sARN,string sPROJ_DESC,string sPERMIT_CODE,string sPERMIT_NO,
    ...
    public class ApplicationQueList
    {
        private List<ApplicationQue> m_lstApp;
        public static ConnectionString dbConn = new ConnectionString();

        public ApplicationQueList()
        {
            m_lstApp = new List<ApplicationQue>();
            string sQuery = string.Empty;

            //using (EPSEntities.EPSEntities db = new EPSEntities.EPSEntities())
            using (var db = new EPSConnection(dbConn))
            {
                sQuery = $"select * from application_que";
                var epsrec = db.Database.SqlQuery<APPLICATION_QUE>(sQuery);

                foreach (var items in epsrec)
                {
                    m_lstApp.Add(new ApplicationQue(items.ARN, items.PROJ_DESC, items.PERMIT_CODE,
        ...
        public static List<APPLICATION_QUE> GetApplicationQue(string sWhereCond)
        {
            using (var db = new EPSConnection(dbConn))
            {
                string sQuery = string.Empty;
                ...
                if(!string.IsNullOrEmpty(sWhereCond))
                    sQuery += $" {sWhereCond}";
                return db.Database.SqlQuery<APPLICATION_QUE>(sQuery).ToList();
            }
        }

        public List<ApplicationQue> AppLst
        {
            get { return m_lstApp; }
            set { m_lstApp = value; }
        }
