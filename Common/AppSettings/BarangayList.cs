...
namespace Common.AppSettings
{
    ...
    public class BarangayList
    {
        ...
        public void GetBarangayList(string strDistrictCode, bool blnIsOrderByName)
        {
            ...
            strTmpDistrictCode = strDistrictCode;
            OracleResultSet result = new OracleResultSet();
            StringBuilder strQuery = new StringBuilder();
            strQuery.Append("SELECT dist_code, brgy_code, brgy_nm FROM brgy WHERE 1=1");

            if (strTmpDistrictCode == "01" && strDistrictCode == "")
            {
                m_strDistrictCode = "";
            }
            else if (strTmpDistrictCode == "01")
            {
                m_strDistrictCode = strDistrictCode;
                strQuery.Append(string.Format(" AND dist_code = '{0}'", strDistrictCode));
            }
            //else if (strTmpDistrictCode != "")   // RMC 20100624
            else if (strTmpDistrictCode != "" && strDistrictCode != null)
            {
