        ...
        private int getDocCodeMethod(string strTRN)
        {
            try
            {
                ...
                if (AppSettingsManager.GetBlobImageConfig() == "F")
                    ...
                    result.Query = string.Format("select DECODE(max(doc_code) + 1, NULL, 1, max(doc_code) + 1) from docblob_twopage where arn = '{0}' and sys_type = '{1}'", strTRN, AppSettingsManager.GetSystemType); //eps
                    ...
                    result.Query = string.Format("select DECODE(max(doc_code) + 1, NULL, 1, max(doc_code) + 1) from docblob_tbl where arn = '{0}' and sys_type = '{1}'", strTRN, AppSettingsManager.GetSystemType); //eps
                int intDocCode = 1;
