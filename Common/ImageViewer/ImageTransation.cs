...
namespace Amellar.Common.ImageViewer
{
    public class ImageTransation
    {
        ...
        public bool UpdateImage(ImageInfo objInfo)
        {
            try
            {
                ...
                //{
                    ...
                    if (AppSettingsManager.GetBlobImageConfig() == "F") //for eps temp
                        //result.Query = "update docblob_twopage set iscancelled = 0, ispending = 0, isrejected = 0, ARN = :1, doc_code = :2, dist_code = :3, kind = :4, deficient = :5, upload_dt = :6, upload_tm = :7, upload_by = :8, status = :9 where id = :10 and encoder = :11 and sys_type = :12";
                        result.Query = "update docblob_twopage set iscancelled = 0, ispending = 0, isrejected = 0, ARN = :1, doc_code = :2, dist_code = :3, kind = :4, deficient = :5, upload_dt = :6, upload_tm = :7, upload_by = :8, status = :9 where encoder = :10 and sys_type = :11 and (id = :12 or id = :13)"; // for eps twopage
                    //else// RMC 20150226 adjustment in blob configuration (e)
                        ...
                        result.Query = "update docblob_tbl set iscancelled = 0, ispending = 0, isrejected = 0, ARN = :1, doc_code = :2, dist_code = :3, kind = :4, deficient = :5, upload_dt = :6, upload_tm = :7, upload_by = :8, status = :9 where id = :10 and encoder = :11 and sys_type = :12"; //EPS
                ...
                //result.AddParameter(":10", intId + i);
                    ...
                    result.AddParameter(":11", objInfo.System);
                    result.AddParameter(":12", intId);
                    result.AddParameter(":13", intId + 1); // FOR EPS TWOPAGE
        ...
        */
        ...
        public bool DetachImage(string m_sTRN)
        {
            try
            {
                ...
                if (m_strFileName.Contains("BPLS ASSESSMENT - APPLICATION")) // AST 20150430
                    ...
                    result.Query = "select count(*) from docblob_tbl where ARN = :1"; //EPS
                // AST 20150415 remove condition as gob request (e)
                ...
                if (intCount != 0)
                {
                    ...
                    if (m_strFileName.Contains("BPLS ASSESSMENT - APPLICATION")) // AST 20150430
                        ...
                        result.Query = string.Format("update docblob_tbl set ARN = '', deficient = '', status = '', encoder =:1 where ARN = :2 and sys_type = '{0}'", AppSettingsManager.GetSystemType); //EPS
                    // AST 20150415 remove condition as gob request (e)
        ...
        public bool DetachImage(string m_sTRN, int intID)
        {
            try
            {
                ...
                if (m_strFileName.Contains("BPLS ASSESSMENT - APPLICATION")) // AST 20150430
                    ...
                    result.Query = "select count(*) from docblob_twopage where ARN = :1 and (id = :2 or id = :3)"; // for EPS
                // AST 20150415 remove condition as gob request (e)
                result.AddParameter(":1", m_sTRN);
                ...
                if (intCount != 0)
                {
                    ...
                    if (m_strFileName.Contains("BPLS ASSESSMENT - APPLICATION")) // AST 20150430
                        ...
                        result.Query = string.Format("update docblob_twopage set ARN = '',doc_code = '', deficient = '', status = '', encoder =:1 where ARN = :2 and (id = :3 or id =:4) and sys_type = '{0}'", AppSettingsManager.GetSystemType); // for EPS
                    // AST 20150415 remove condition as gob request (e)

                    result.AddParameter(":1", AppSettingsManager.SystemUser.UserCode);
                        result.AddParameter(":2", m_sTRN);
                        //result.AddParameter(":2", intID + i);
                        result.AddParameter(":3", intID);
                        result.AddParameter(":4", intID +1); //for eps
                    result.ExecuteNonQuery();
                    //}
                    // AST 20150415 remove loop temporary for work around daw po(e)
        ...
        public bool InsertImage(string sArn, string sFileDirectory, string sBrgy)
        {
            // RMC 20111206
            try
            {
                ...
                // RMC 20150226 adjustment in blob configuration (s)
                if (sBlobConfig == "F")
                    result.Query = "insert into docblob_twopage values ('" + sArn + "', ";
                else// RMC 20150226 adjustment in blob configuration (e)
                    result.Query = "insert into docblob_tbl values ('" + sArn + "', ";
