...
namespace Common.DataConnector
{
    /// <summary>
    ...
    public class OracleResultSet
    {
        ...
        OracleDataConnector objOracleDataConn; // new instance of connection

        //JVL added this class inside the OracleResult class(s)

        /// <summary>
        /// This method create a new instance
        /// 
        /// </summary>
        /// <param name="intType">0 new blob connection, 1 new instance, 2 new database connection</param>
        public void CreateNewInstance(int intType)
        {
            //if (ConnectionType == ConnectionTypes.NewOracleConnection)
            if (intType == 1)
            {

                int intPort = 0;
                int.TryParse(System.Configuration.ConfigurationManager.AppSettings["Port"], out intPort);
                objOracleDataConn = new OracleDataConnector(
                           System.Configuration.ConfigurationManager.AppSettings["Host"],
                           intPort,
                ...
                //objOracleResultSet = new OracleResultSet();

                //objOracleResultSet.CreateInstance(objOracleDataConn.Connection);

                this.CreateInstance(objOracleDataConn.Connection);

            }

            //else if (ConnectionType == ConnectionTypes.BlobConnection)
            else if (intType == 0)
            {
                int intPort = 0;
            ...
            //else if (ConnectionType == ConnectionTypes.NewDatabaseConnection)
            else if (intType == 2)
            {
                //not yet implemented
            }
        ...
        public bool isConnected
        {
            get
            {
                return (m_cmdCommand.Connection.State == ConnectionState.Open);
            }
        }

        public bool CloseExtendedInstance()
        {
            objOracleDataConn = null; //force closing of extended instance
            return true;
        }
